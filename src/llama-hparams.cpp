...
void llm_load_hparams(
    ...
    if (hparams.n_head() > 0) {
        ...
        if (model.arch == LLM_ARCH_LLAMA || model.arch == LLM_ARCH_LLAMA_EMBED || model.arch == LLM_ARCH_FALCON || model.arch == LLM_ARCH_BITNET_25 || model.arch == LLM_ARCH_BITNET_B158 || model.arch == LLM_ARCH_DECI) {
